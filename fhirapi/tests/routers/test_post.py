...
def mock_generate_cute_creature_api(mocker):
    return mocker.patch(
        "fhirapi.tasks._generate_cute_creature_api",
        return_value={"output_url": "https://fakeurl.cm/image.jpg"},
    )
...
async def test_create_post_with_prompt(
    ...
    body = "Test post"
    response = await async_client.post(
        "/post?prompt=A cute newfoundland puppy with a pink bow on its head",
        json={"body": body},
        headers={"Authorization": f"Bearer {logged_in_token}"},
    )
    assert response.status_code == 201
...
@pytest.mark.anyio
async def test_create_post_w_expired_token(
...
async def test_get_all_posts_sorted_by_likes(
    ...
    await create_post("Test Post", async_client, logged_in_token)
...
@pytest.mark.anyio
async def test_get_all_posts_invalid_sorting(
    async_client: AsyncClient,
):
    response = await async_client.get("/post", params={"sorting": "nope"})
    assert response.status_code == 422
